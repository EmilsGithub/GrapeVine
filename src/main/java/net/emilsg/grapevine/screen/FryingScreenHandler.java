...
public class FryingScreenHandler extends AbstractRecipeScreenHandler<RecipeInputInventory>  {
    ...
    public FryingScreenHandler(int syncId, PlayerInventory playerInventory, ScreenHandlerContext context) {
        super(ModScreenHandlers.FRYING, syncId);
        int j;
        int i;
        ...
        this.addSlot(new Slot(this.input, 0, xPos, yPos));
        this.addSlot(new Slot(this.input, 1, xPos + 18, yPos));
        this.addSlot(new Slot(this.input, 2, xPos + 36, yPos));
        this.addSlot(new Slot(this.input, 3, xPos + 54, yPos));
    ...
    protected static void updateResult(ScreenHandler handler, World world, PlayerEntity player, RecipeInputInventory craftingInventory, CraftingResultInventory resultInventory) {
        ...
        Optional<ShapelessFryingRecipe> optional = world.getServer().getRecipeManager().getFirstMatch(ModRecipeSerializers.FryingType.INSTANCE, craftingInventory, world);

        if (optional.isPresent()) {
            ShapelessFryingRecipe fryingRecipe = optional.get();
            ItemStack itemStack = fryingRecipe.craft(craftingInventory, world.getRegistryManager());
    ...
    public void onContentChanged(Inventory inventory) {
        this.context.run((world, pos) -> FryingScreenHandler.updateResult(this, world, this.player, this.input, this.result));
    }
    ...
    public boolean canUse(PlayerEntity player) {
        return FryingScreenHandler.canUse(this.context, player, ModRegistries.FRYING_PAN);
    }
